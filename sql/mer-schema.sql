...
-- Koppeling MER-traject -> onderliggende regeling (OCD). Zacht (geen harde
-- KOOP<->AKN-sleutel): bronhouder (op bestuurslaag) + documenttype/titel voor Ow
-- (p2p.regeling.frbr_work) resp. naam-match voor Wro (wro.ruimtelijk_instrument.idn).
-- Gevuld door build_regeling_links.py.
CREATE TABLE IF NOT EXISTS mer.project_regeling (
    project_slug  text NOT NULL REFERENCES mer.project (slug) ON DELETE CASCADE,
    doel_type     text NOT NULL,          -- 'ow' | 'wro'
    doel_id       text NOT NULL,          -- p2p frbr_work (ow) of wro IMRO-idn (wro)
    opschrift     text,
    documenttype  text,
    methode       text,
    zekerheid     numeric(3,2),
...
CREATE INDEX IF NOT EXISTS idx_mer_projreg_project ON mer.project_regeling (project_slug);
